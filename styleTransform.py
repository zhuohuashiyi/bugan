...
logging.basicConfig(filename='styleTransform.log', level=logging.DEBUG, format=LOG_FORMAT)
...
def train(X, contents_Y, styles_Y, lr, max_epochs, lr_decay_epoch):
    ...
    for i in range(max_epochs):
        ...
        logging.info('epoch %3d, content loss %.2f, style loss %.2f, '
                  'TV loss %.2f, %.2f sec'
                  % (i + 1, nd.add_n(*contents_l).asscalar(),
                     nd.add_n(*styles_l).asscalar(), tv_l.asscalar(),
                     time.time() - start))
        if i % lr_decay_epoch == 0 and i != 0:
            trainer.set_learning_rate(trainer.learning_rate * 0.1)
            logging.info('change lr to %.1e' % trainer.learning_rate)
    return X
...
if __name__ == "__main__":
	...
	plt.imsave('neural-style-2.png', postprocess(output).asnumpy())
